...
fn lookup(db :: conn.ConnDb, symbol :: Str) -> [ref_data, sql] Result[rd.Instrument, dbe.DbErr] {
  let sq := q.for_dialect({ sql: "SELECT symbol, isin, cusip, lei, name, currency, exchange, asset_class FROM instruments WHERE symbol = ?", params: [PStr(symbol)] }, db.dialect)
  let raw :: Result[List[{ symbol :: Str, isin :: Str, cusip :: Str, lei :: Str, name :: Str, currency :: Str, exchange :: Str, asset_class :: Str }], SqlError] := sql.query(db.handle, sq.sql, sq.params)
  match raw {
    Err(e) => Err(dbe.sql_error(match e.code {
      None => "",
      Some(c) => c,
...
fn upsert_instrument(db :: conn.ConnDb, inst :: rd.Instrument) -> [ref_data, sql] Result[Unit, dbe.DbErr] {
  ...
  let sq := q.for_dialect({ sql: "INSERT INTO instruments (symbol, isin, cusip, lei, name, currency, exchange, asset_class) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (symbol) DO UPDATE SET isin = EXCLUDED.isin, cusip = EXCLUDED.cusip, lei = EXCLUDED.lei, name = EXCLUDED.name, currency = EXCLUDED.currency, exchange = EXCLUDED.exchange, asset_class = EXCLUDED.asset_class", params: [PStr(inst.symbol), PStr(inst.isin), PStr(inst.cusip), PStr(inst.lei), PStr(inst.name), PStr(inst.currency), PStr(inst.exchange), PStr(ac_str)] }, db.dialect)
...
fn remove(db :: conn.ConnDb, symbol :: Str) -> [ref_data, sql] Result[Unit, dbe.DbErr] {
  let sq := q.for_dialect({ sql: "DELETE FROM instruments WHERE symbol = ?", params: [PStr(symbol)] }, db.dialect)
  match sql.exec(db.handle, sq.sql, sq.params) {
    Err(e) => Err(dbe.sql_error(match e.code {
      None => "",
      Some(c) => c,
...
fn decode_row(row :: { symbol :: Str, isin :: Str, cusip :: Str, lei :: Str, name :: Str, currency :: Str, exchange :: Str, asset_class :: Str }) -> rd.Instrument {
  { symbol: row.symbol, isin: row.isin, cusip: row.cusip, lei: row.lei, name: row.name, currency: row.currency, exchange: row.exchange, asset_class: decode_asset_class(row.asset_class) }
}
