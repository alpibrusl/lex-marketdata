...
fn aapl_instrument() -> rd.Instrument {
  { symbol: "AAPL", isin: "US0378331005", cusip: "037833100", lei: "HWUPKR0MPOU8FGXBT394", name: "Apple Inc.", currency: "USD", exchange: "NASDAQ", asset_class: rd.Equity }
}

fn msft_instrument() -> rd.Instrument {
  { symbol: "MSFT", isin: "US5949181045", cusip: "594918104", lei: "INR2EJN1ERAN0W5ZP974", name: "Microsoft Corporation", currency: "USD", exchange: "NASDAQ", asset_class: rd.Equity }
}
...
fn test_upsert_updates_existing() -> [ref_data, sql, fs_write] Result[Unit, Str] {
  match open_db() {
    ...
    Ok(db) => match rds.upsert_instrument(db, aapl_instrument()) {
      ...
      Ok(_) => {
        let updated := { symbol: "AAPL", isin: "US0378331005", cusip: "037833100", lei: "HWUPKR0MPOU8FGXBT394", name: "Apple Inc. (Updated)", currency: "USD", exchange: "NASDAQ", asset_class: rd.Equity }
...
fn test_asset_class_round_trip() -> [ref_data, sql, fs_write] Result[Unit, Str] {
  match open_db() {
    Err(msg) => fail(msg),
    Ok(db) => {
      let etf := { symbol: "SPY", isin: "US78462F1030", cusip: "78462F103", lei: "", name: "SPDR S&P 500 ETF Trust", currency: "USD", exchange: "NYSE", asset_class: rd.ETF }
...
fn suite() -> [ref_data, sql, fs_write] List[Result[Unit, Str]] {
  [test_lookup_unknown_returns_not_found(), test_upsert_and_lookup(), test_upsert_updates_existing(), test_two_instruments_isolated(), test_asset_class_round_trip(), test_remove()]
}
